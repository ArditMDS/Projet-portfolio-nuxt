<template>
    <div class="containerHeaderHomepage">
        <div class="containerDroitHP">
          <div class="divBac">
            <img class="imageHeader" src="../public/image/moiPetit.png" alt="">
            <span class="popUpImage">Bac +3</span>
          </div>
        </div>
        <div class="containerGaucheHeaderHP">
            <h1 class="headerHP">Je m'appelle <strong>Ardit Mataj</strong></h1>
            <p class="descHomepage">Je suis étudiant en développement Web à MyDigitalSchool Annecy. Je cherche une alternance en développement web (sur tous les domaines ce qui ce trouve si dessous)</p>
            <div class="containerSkills">
                <div class="skill">Html / Css</div>
                <div class="skill">VueJS</div>
                <div class="skill">Symfony</div>
                <div class="skill">Php</div>
                <div class="skill">Wordpress</div>
                <div class="skill">Suite Adobe</div>
                <div class="skill">C#</div>
            </div>
        </div>
        
    </div>
</template>